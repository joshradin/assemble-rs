...
impl<T, R> Clone for FnProvider<T, R>
where
    R: Into<Option<T>>,
    T: Send + Sync + Clone,
{
...
impl<T, R> Debug for FnProvider<T, R>
where
    R: Into<Option<T>>,
    T: Clone + Send + Sync,
{
...
impl<T, R, F, P> Debug for Map<T, R, F, P>
where
    F: Fn(T) -> R + Send + Sync,
    P: Provider<T>,
    R: Clone + Send + Sync,
...
impl<T, R, PT, PR, F> Debug for FlatMap<T, R, PT, PR, F>
where
    F: Fn(T) -> PR + Send + Sync,
    PR: Provider<R>,
    PT: Provider<T>,
...
impl<T, B, R, F> Debug for Zip<T, B, R, F>
where
    B: Clone + Send + Sync,
    F: Fn(T, B) -> R + Send + Sync,
    R: Clone + Send + Sync,
