...
impl Input {
    ...
    pub fn input_changed(&self, prev: Option<Input>) -> bool {
        if let Some(prev) = prev {
            ...
            self.serialized_data != prev.serialized_data
        } else {
            true
        }
    }
...
impl WorkHandler {
    ...
    pub fn try_get_prev_input(&self) -> Option<Input> {
        ...
        if file_location.exists() {
            ...
            read.read_to_string(&mut buffer).expect(&format!("Could not read to end of {:?}", file_location));
    ...
    pub fn add_input<T : Serialize + Send + Sync + Clone + 'static, P : IntoProvider<T>>(&mut self, id: &str, value: P) -> ProjectResult
    ...
    pub fn add_prop<T : Serialize + Send + Sync + Clone + 'static>(&mut self, prop: &Prop<T>) -> ProjectResult {
    ...
    pub fn get_input(&self) -> &Input {
        self.final_input.get_or_init(|| {
            Input::new(&self.task_id, &self.inputs)
        })
    }
...
impl Serialize for InputFile {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
        if self.0.exists() {
            let mut read = File::open(&self.0).expect(&format!("Dont have read access to {:?}", self.0));
            let mut buffer = Vec::new();
            read.read_to_end(&mut buffer).expect(&format!("Could not read to end of {:?}", self.0));
...
pub fn normalize_system_time(system_time: SystemTime) -> OffsetDateTime {
    let duration = system_time.duration_since(SystemTime::UNIX_EPOCH).expect("Couldn't determine duration since UNIX EPOCH");
